...
connection_string_url = os.environ.get("CONNECTION_URL")
...
with PostgresSaver.from_conn_string(
    conn_string=connection_string_url
) as pgcheckpointer:
    # checkpointer.setup()
    app = builder.compile(checkpointer=pgcheckpointer)

    test = {"messages": []}
    config1 = {"configurable": {"thread_id": "2a"}}
    while True:
        user_inp = input()
        if user_inp == "EXIT":
            break
            ...
            app.invoke(test, config=config1)
